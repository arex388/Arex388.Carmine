...
- **Fixed:** Live API responses could fail to deserialize entirely (every list operation returning `Failed` with a `JsonException`): async stream deserialization invoked the converters on partially buffered JSON, where skipping an unknown property throws. The client now buffers the response body before deserializing, making the parse safe on every runtime.
- **Fixed:** Cancelling a request while the response body downloads now reliably returns the `Cancelled` response on every runtime: the body buffers inside `GetAsync` (default `ResponseContentRead`), governed by the caller's cancellation token and `HttpClient.Timeout`, instead of after a headers-read completion whose body read netstandard2.0 cannot cancel.
- **Removed:** The unused `System.Net.Http.Json` package dependency (superseded by direct `JsonSerializer` deserialization over the buffered body) and the explicit `Microsoft.SourceLink.GitHub` reference (the .NET SDK includes SourceLink automatically) — the published package's dependency list shrinks accordingly.
- **Added:** XML docs on `Trip` and `Vehicle` noting that the lazily cached unit-conversion properties are unsynchronized on first access — don't share an instance across threads while they are first read.
...
- **Refactored:** `CarmineClient`'s six request pipelines consolidated into one generic core with static, compiler-cached mapper lambdas — no public or behavioral change.
- **Optimized:** Hidden request-path allocations trimmed for netstandard2.0 — get-endpoint building no longer boxes the id struct through `string.Format`, the trip converter no longer allocates discarded list locals, and the derived unit-conversion factors are centralized in a single internal constants class.
...
- **Updated:** FluentValidation pinned to `[11.12.0,12.0.0)` (v12 dropped .NET Standard 2.0); NetEscapades.EnumGenerators to 1.0.0-beta21 (build is now warning-free); PolySharp to 1.16.0; Microsoft.Extensions.* / System.Text.Json to 10.0.10 — consumers on the 9.x Microsoft.Extensions stack should stay on 4.1.2.
- **Added:** README deployment guidance for long-running services — configuring `SocketsHttpHandler.PooledConnectionLifetime` on the named client for DNS freshness — and XML docs on the `ICarmineClient` extensions stating that a failed request is indistinguishable from an empty result (`null` / empty list).
- **Refactored:** Inert JSON serialization metadata removed — the models' `[JsonPropertyName]` attributes and the serializer's naming/number-handling options were never consulted, since every model type deserializes exclusively through the hand-written converters; a guard test now fails if a model type lacks a registered converter.
